...
const SearchExercises = ({ setExercises, bodyPart, setBodyPart }) => {
  ...
  const handleSearch = async () => {
    if (search) {
      const exercisesData = await fetchData(`https://exercisedb.p.rapidapi.com/exercises/name/${search}?offset=0&limit=100`, excerciseOptions);
  ...
  >
        <Horizontal data={bodyParts} bodyParts setBodyPart={setBodyPart} bodyPart={bodyPart} />
      </Box>
    </Stack>
  );
};
